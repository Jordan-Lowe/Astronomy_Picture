...
import { getFirestore, collection, getDoc, doc, updateDoc, increment, setDoc, getDocs, limit, orderBy, query, } from 'firebase/firestore';
...
export async function fetchPopularImages() {
    ...
    const q = query(collection(db, 'nasaData'), orderBy('clicks', 'desc'), limit(9));
    ...
    querySnapshot.forEach((doc) => {
        popularImages.push(doc.data());
    });
    // Return popular images array
    return popularImages;
}
export async function fetchLastImages() {
    const latestImages = [];
    for (let i = 0; i < 9; i++) {
        const date = new Date();
...
export async function fetchDailyImage(dateToFetch) {
...
export async function incrementClickCount(data) {
    if (data.date) {
        try {
